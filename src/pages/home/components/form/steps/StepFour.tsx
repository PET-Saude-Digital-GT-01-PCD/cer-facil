...
import { matchCers, type MatchingResult } from "@/lib/matchCers";
...
export default function StepFour({
  ...
  useEffect(() => {
    ...
    setResults(matchCers(deficiencies, location));
    setLoading(false);
  ...
  return (
    <div className="w-full">
      <Card className="border-2 border-[var(--cor-bg-1)] shadow-2xl max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle asChild>
            <h2 className="text-xl text-[var(--cor-bg-1)] font-bold">
              Resultados
            </h2>
          </CardTitle>
                  ...
                  >
                    <CardContent aria-hidden="true" className="p-5">
                      <div className="flex gap-3 items-center mb-2">
                        ...
                        <h3 className="font-bold text-xl text-[var(--cor-bg-1)]">{result.cer.nome}</h3>
                      </div>
                      <div className="pl-11 flex items-center gap-3 text-base text-muted-foreground mb-4">
                      ...
                      <div className="pl-11 flex justify-between items-center">
                        <div className="flex flex-wrap gap-2">
                          {result.cer.especialidades.map((esp, i) => (
                            <span key={i} className="bg-gray-100 px-2 py-1 rounded text-base font-medium">
                              {esp}
                            </span>
                          ))}
                        </div>
                        <Button
                          ...
                          size="lg"
                          className="ml-4 shrink-0 text-base border-2 border-[var(--cor-bg-1)] bg-transparent text-[var(--cor-bg-1)] hover:bg-[var(--cor-bg-1)] hover:text-white focus-visible:ring-[10px] focus-visible:ring-[var(--cor-destaque)] focus-visible:ring-offset-2 outline-none"
                        >
                          Saiba mais
                        </Button>
                      </div>
