      ...
      <div className="mx-auto max-w-3xl w-full">
        <div className="text-left mb-6 flex justify-between items-end">
          ...
          <Button
            variant="outline"
            size="sm"
            className="hidden md:flex text-xl text-[var(--cor-3)] border-[var(--cor-3)] hover:bg-[var(--cor-3)] hover:text-white transition-all"
        ...
        <div className="flex flex-col gap-5">
          <div className="bg-white p-5 rounded-xl shadow-md border border-blue-100 flex flex-col items-center text-center">
            <Icone className="w-12 h-12 text-[var(--cor-3)] mb-3" />
          ...
          <div className="bg-white p-5 rounded-xl shadow-md border border-blue-100 space-y-4">
            ...
            {cerInfo?.endereco && (
              <div className="mb-2">
                <p className="text-xl font-bold text-[var(--cor-3)] uppercase tracking-widest mb-1 flex items-center gap-2">
            ...
            <div className="relative border-l-2 border-blue-100 ml-3 space-y-4">
              {fluxoInfo.steps?.map((step: any, index: number) => (
                <div key={index} className="relative pl-8">
                  <div className="absolute -left-[12px] top-1 w-6 h-6 rounded-full bg-white border-[3px] border-[var(--cor-3)] shadow-sm" />
                  <div className="bg-gray-50/50 p-3 rounded-lg border border-gray-100 transition-all hover:shadow-sm hover:border-blue-100">
                    <div className="flex items-center gap-2 mb-1">
                      <Calendar className="w-6 h-6 text-[var(--cor-3)]" />
                      <h4 className="font-bold text-xl text-gray-800 leading-tight">{step.title}</h4>
                    </div>
          ...
          <div className="bg-white p-5 rounded-xl shadow-md border border-blue-100">
            ...
            <ul className="space-y-2">
              {fluxoInfo.documents?.map((doc: string, i: number) => (
                <li key={i} className="flex items-start text-xl text-gray-700 leading-snug">
                  <span className="mr-2 text-[var(--cor-3)] font-black text-2xl leading-none">•</span>
                  {doc}
                </li>
              ))}
            </ul>
        ...
        <div className="mt-6 flex justify-center md:hidden">
          <Button
            size="sm"
            className="w-full text-white bg-[var(--cor-3)] hover:bg-orange-600 transition-all text-xl py-5 rounded-xl"
            onClick={() => setShowFlow([false, cerId])}
          >
            Voltar para a busca
          </Button>
